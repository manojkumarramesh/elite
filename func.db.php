...
function db_query($query) {
	...
	if (!$result && $mysql_autorepair && preg_match("/(?:'(\S+)\.(?:MYI|MYD))|(?:Table\s+'(\S+)'\s+is\s+marked\s+as\s+crashed\s+and\s+should\s+be\s+repaired)/", $mysql_error, $m)) {	
		$m = $m[1];
		...
		if (!$result) {
			error_log('Repaire table ' . $m . ' is failed: ' . $mysql_error, 0);
...
#
function func_query_first($query, $use_cache = false) {
...
#
function func_query_first_cell($query, $use_cache = false) {
...
function func_array2update ($tbl, $arr, $where = '') {
	...
	foreach ($arr as $k => $v) {
		...
		if ((strlen($v) == 0) || (!(($v{0} == '"') && ($v{strlen($v)-1} == '"'))) && 
			(!(($v{0} == "'") && ($v{strlen($v)-1} == "'")))) {
			$v = "'$v'";
		}
		$r[] = "$k=$v";
...
function func_generate_joins($joins, $parent = false) {
	...
	foreach ($joins as $jname => $j) {
		if (!isset($j['parent'])) $j['parent'] = false;
		if ((empty($parent) && !empty($j['parent'])) || (!empty($parent) && $parent != $j['parent']))
			continue;
...
function db_connection ($_sql_host = "", $_sql_user = "", $_sql_db = "", $_sql_password = "") {
	...
	if (!$sql_connection || !$sql_select_db) {
		$config_php = "<b><a href=\"http://www.x-cart.com/xcart_manual/online/?configuration_files.htm\">config.php</a></b>";
		if ($debug_mode > 1)
			func_direct_log('sql', $error_text);
			func_show_error_page("Cannot connect to MySQL server", $web_error_text,
...
function db_error_msg($web = false) {
	...
	return ($web) ? '<a href="http://search.mysql.com/search?q=' . mysql_errno() . '">' . mysql_errno() . '</a>' . ": " . mysql_error() . "<br /><br />\n" : mysql_errno() . ': ' . mysql_error(). "\n";
}
