 ...
 * @version    $Id: image_selection.php,v 1.79.2.3 2011/08/17 07:32:29 aim Exp $
...
$allowed_formats = array('image/pjpeg', 'image/jpeg', 'image/jpg');//Allowed formats for images
...
if ($REQUEST_METHOD == 'POST') {
    ...
    case 'L': // uploaded file
        ...
        if(!in_array($_FILES['userfile']['type'], $allowed_formats)) {
    ...
    if($type == 'P' && $source != 'U')
    {
        if($data['image_x'] < MINIMUM_ALLOWED_WIDTH) {
            $top_message['content'] = func_get_langvar_by_name("err_upload_min_allowed_width");
            $top_message['type'] = 'W';
            func_header_location($HTTP_REFERER);
        }

        if($data['image_y'] < MINIMUM_ALLOWED_HEIGHT) {
            $top_message['content'] = func_get_langvar_by_name("err_upload_min_allowed_height");
            $top_message['type'] = 'W';
            func_header_location($HTTP_REFERER);
        }
        
        if($data['file_size'] > MAXIMUM_IMAGE_UPLOAD_LIMIT) {
            $top_message['content'] = func_get_langvar_by_name("err_upload_max_image_size");
            $top_message['type'] = 'W';
            func_header_location($HTTP_REFERER);
        }
    }
    elseif($type == 'C')
    {
        if($data['image_x'] < $config['images_dimensions']['C']['width'] )
        {
            $top_message['content'] = func_get_langvar_by_name("err_upload_category_min_allowed_width");
            $top_message['type'] = 'W';
            func_header_location($HTTP_REFERER);
        }

        if($data['image_y'] < $config['images_dimensions']['C']['height'])
        {
            $top_message['content'] = func_get_langvar_by_name("err_upload_category_min_allowed_height");
            $top_message['type'] = 'W';
            func_header_location($HTTP_REFERER);
        }
    }
...
if($type == 'P')
{
    $upload_warning_message = func_get_langvar_by_name('txt_product_max_file_size_warning');
}
